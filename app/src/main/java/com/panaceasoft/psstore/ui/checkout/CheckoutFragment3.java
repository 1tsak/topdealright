...
public class CheckoutFragment3 extends PSFragment implements DataBoundListAdapter.DiffUtilDispatchedInterface {
    ...
    protected void initUIAndActions() {
        ...
        */
        ...
        binding.get().razorCardView.setOnClickListener(v -> {
            if (!clicked) {
                clicked = true;
                oldCardView = binding.get().razorCardView;
                oldImageView = binding.get().razorImageView;
                oldTextView = binding.get().razorTextView;
                changeToOrange(oldCardView, oldTextView, oldImageView);
            } else {
                if (oldCardView != null && oldImageView != null && oldTextView != null) {
                    changeToWhite(oldCardView, oldTextView, oldImageView);
                    changeToOrange(binding.get().razorCardView, binding.get().razorTextView, binding.get().razorImageView);
                    oldCardView = binding.get().razorCardView;
                    oldImageView = binding.get().razorImageView;
                    oldTextView = binding.get().razorTextView;
                }
            ...
            binding.get().warningTitleTextView.setText(R.string.checkout__information__STRIPE);
            paymentMethod = Constants.PAYMENT_RAZOR;
        });

        binding.get().paytmCardView.setOnClickListener(v -> {
            if (!clicked) {
                clicked = true;
                oldCardView = binding.get().paytmCardView;
                oldImageView = binding.get().paytmImageView;
                oldTextView = binding.get().paytmTextView;
                changeToOrange(oldCardView, oldTextView, oldImageView);
            } else {
                if (oldCardView != null && oldImageView != null && oldTextView != null) {
                    changeToWhite(oldCardView, oldTextView, oldImageView);
                    changeToOrange(binding.get().paytmCardView, binding.get().paytmTextView, binding.get().paytmImageView);
                    oldCardView = binding.get().paytmCardView;
                    oldImageView = binding.get().paytmImageView;
                    oldTextView = binding.get().paytmTextView;
                }
            ...
            binding.get().warningTitleTextView.setText(R.string.checkout__information__STRIPE);
            paymentMethod = Constants.PAYMENT_PAYTM;
        });
...
//                progressDialog.show();
        ...
        }else if (requestCode == Constants.REQUEST_CODE__RAZOR_ACTIVITY && resultCode == Constants.RESULT_CODE__RAZOR_ACTIVITY) {
            if (this.getActivity() != null) {
...
//                progressDialog.show();
        ...
        }else if (requestCode == Constants.REQUEST_CODE__PAYTM_ACTIVITY && resultCode == Constants.RESULT_CODE__PAYTM_ACTIVITY) {
            if (this.getActivity() != null) {
